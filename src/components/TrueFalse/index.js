...
class TrueFalseCard extends Component {
  constructor(props) {
    super();
    this.state = {
      switch: true,
  ...
  render() {
    ...
    return (
      <Col span={24}>
        <Card title={"Setup" + (this.props.index + 1)}>
          <Row>
...
class TrueFalseForm extends Component {
  ...
  RenderTrueFalseCard = (props) => {
    let a = [];
    for (var i = 0; i < props.elements; i++) {
      a.push(<TrueFalseCard
        key={i}
        index={i}
  ...
  render() {
    return (
      <div>
        {
          this.RenderTrueFalseCard(this.props)
        }
      </div>
    )
  }
